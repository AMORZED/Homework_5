...
namespace SStreamWorker
{
	...
	public class StreamWorker : IStreamWorker
	{
		...
		public void FindAndReplaceASpecificWord(string path, string searchedWord, string replacebleWord)
		{
			string originalText;

			using (StreamReader sr = new StreamReader(path))
			{
				originalText = sr.ReadToEnd(); // Запись исходного текста в переменную.
			}
